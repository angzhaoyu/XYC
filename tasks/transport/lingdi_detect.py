...
class LingdiDetector:
    ...
    def _match_number(self, screenshot, prefix, values):
        ...
        # 用第一个存在的模板确定搜索区域（scale=2.0 保证搜索区 > 模板）
        limit = None
        for v in values:
            path = os.path.join(self.RIMGES_DIR, f"{prefix}_{v}.png")
            if os.path.exists(path):
                limit = self.vision.limit_scope(path, scale=8.0)
                break
        if limit is None:
        ...
        print(f"  {prefix} → {best_val} (conf={best_conf:.3f})")
        return best_val
